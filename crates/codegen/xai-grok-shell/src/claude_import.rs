...
pub enum ImportScope {
    /// User-level: writes to `~/.thanh/config.toml`.
    Global,
...
pub struct ImportPlan {
    /// Items to write to `~/.thanh/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.thanh/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.thanh/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.thanh/config.toml`.
///
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.thanh/hooks/imported-from-claude.json.
...
mod tests {
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.thanh) don't interfere.
...
"#;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.thanh/config.toml` may legitimately have the marker
