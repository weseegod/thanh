...
/// auth.json scope key for plain API key auth (desktop login, `thanh login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from grok-desktop login or `thanh login --api-key`)
    ApiKey,
}
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `thanh login --legacy`
