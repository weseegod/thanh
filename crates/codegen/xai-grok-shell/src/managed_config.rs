...
/// files (TUI tick vs `thanh login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `thanh setup`; runs retries to completion.
    Standard,
...
/// Callers doing auto-fetch should check this; explicit user actions (thanh setup) skip it.
///
...
/// read-only `thanh setup --json` both build on this.
...
/// Post-login hook for `thanh login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `thanh setup` could install config for.
...
/// are the non-forgeable layers. Recovery: reconnect / `thanh setup`; ceasing to serve
...
/// Outcome of the `thanh setup` sync. The caller renders it — CLI presentation
...
/// Result of `thanh setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `thanh setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `thanh setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `thanh setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `thanh setup` would refuse
...
/// Run the `thanh setup` sync for the current principal. The caller must check
