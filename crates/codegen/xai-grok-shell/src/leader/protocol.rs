...
pub struct LeaderCapabilities {
    ...
    /// (driven by `thanh update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `thanh update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `thanh update` just installed; the leader uses
