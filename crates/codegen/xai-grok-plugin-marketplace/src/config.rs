//! Parse marketplace sources from `~/.thanh/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.thanh/` and `~/.claude/`.
...
/// instead of `~/.thanh`/`~/.claude`. Each root is checked for
