...
#[test]
fn test_reinstall_hint_internal_ignores_channel() {
    // Fork ships from GitHub Releases; channel is accepted for API
    // compatibility with upstream call sites and ignored.
    let stable = reinstall_hint("internal", "stable");
    let alpha = reinstall_hint("internal", "alpha");
    let enterprise = reinstall_hint("internal", "enterprise");
    assert_eq!(stable, alpha);
    assert_eq!(stable, enterprise);
    assert!(
...
fn test_reinstall_hint_malformed_channel_falls_back_to_stable() {
    // Free-text config channels never reach the fork hint either.
