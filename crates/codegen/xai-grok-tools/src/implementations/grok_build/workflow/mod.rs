...
pub struct WorkflowToolInput {
    ...
    #[serde(default)]
    #[schemars(
        description = "Name of a registered workflow (built-in, or discovered from the project `.grok/workflows/` or user `~/.thanh/workflows/`). Exactly one of `name`, `script`, or `script_path` must be set."
    )]
