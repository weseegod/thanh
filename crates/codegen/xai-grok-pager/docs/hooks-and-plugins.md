...
2. **settings.json** — `extraKnownMarketplaces` from `~/.thanh/settings.json` or `~/.claude/settings.json`
...
Sources land in `~/.thanh/config.toml`:
...
Or in `~/.thanh/settings.json` / `~/.claude/settings.json`:
