...
- Sessions save automatically. Resume the latest with `thanh -c`, or pick
  one with `/resume` (`F3`).
- Long session getting slow? `/compact` frees context; `/context` shows
  where it's going.
- Automate anything: `thanh -p "summarize new TODOs" --output-format json`
  runs headless — great for scripts and CI.
- Stay current with `thanh update`; see what changed with `/release-notes`.
