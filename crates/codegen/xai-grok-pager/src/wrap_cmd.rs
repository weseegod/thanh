//! `thanh wrap` — run any command in a local PTY that forwards its clipboard.
//!
...
//! run — a single shell-quoted string (`thanh wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("thanh wrap: no command given"))?;
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("thanh wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `thanh wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`thanh wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`thanh wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike `grok ssh`, `thanh wrap` does not gate on the terminal brand: the user
