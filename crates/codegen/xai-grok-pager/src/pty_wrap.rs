//! Local PTY wrapper: the engine behind `thanh wrap` (see [`crate::wrap_cmd`]).
//!
...
///
/// This is the engine behind `thanh wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // detached — `thanh wrap` is short-lived and exits with the child.
